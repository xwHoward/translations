...
原文下一些相关的讨论：
> 有必要注意一下，函数构造是一种隐式的eval，因此在实际环境中遇到非法输入有一定的风险。

> 使用```/<%([^%>]+)?%>/g```这个正则无法正确处理一些特定的字符，因为你不能在代码片段中使用```%```和```>```了。你可以用这个非贪婪的正则：```/<%(.+?)%>/g```。

>遇到类似"formData.email"的数据就会有问题，把正则修改为"if\W|for\W|else\W|...."吧。